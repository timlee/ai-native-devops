...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand(
      "aiNativeDevOps.scaffoldGithubWebhookWorkflows",
      async () => {
    ...
    vscode.commands.registerCommand(
      ...
      (phaseArg?: unknown) => {
        const p = resolvePhaseArg(phaseArg) ?? PHASES.find(ph => ph.key === "build");
        if (!p) return;
        BuildPanel.show(p, context, aiRunner);
      }
    ),

    // Open test workflow panel (Phase 05)
    vscode.commands.registerCommand(
      "aiNativeDevOps.openTestWorkflow",
      (phaseArg?: unknown) => {
        const p = resolvePhaseArg(phaseArg) ?? PHASES.find(ph => ph.key === "test");
        if (!p) return;
        TestPanel.show(p, context);
      }
    ),
...
function captureSink(delegate: AiOutputSink, onChunk: (chunk: string) => void): AiOutputSink {
  return {
    appendAiChunk(chunk: string) { onChunk(chunk); delegate.appendAiChunk(chunk); },
    aiDone()              { delegate.aiDone(); },
    aiError(msg: string)  { delegate.aiError(msg); },
  };
}

async function bumpAndPackage(extensionPath: string): Promise<string> {
...
async function scaffoldGithubWebhookWorkflows(
  context: vscode.ExtensionContext
): Promise<void> {
  const repoRoot = resolveRepoRoot(context);
  ...
  const issuePath = ensureTextFile(repoRoot, issueWorkflowPath, issueWorkflow);
  const failurePath = ensureTextFile(repoRoot, failureWorkflowPath, failureWorkflow);
  const prPath = ensureTextFile(repoRoot, prWorkflowPath, prWorkflow);

  const open = await vscode.window.showInformationMessage(
    "GitHub webhook workflow scaffolding completed.",
    "Open Issue Workflow",
    "Open Failure Workflow",
    "Open PR Workflow"
  );
