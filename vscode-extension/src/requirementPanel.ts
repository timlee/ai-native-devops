...
function buildIssueBody(moduleName: string, reqId: string, sections: Map<string, string>): string {
  const lines: string[] = [
    `> **Module:** ${moduleName} | **ID:** ${reqId}`,
...
export class RequirementPanel {
  ...
  private constructor(
    ...
    this._panel.webview.onDidReceiveMessage(
      (msg: { command: string } & Partial<SubmitMessage> & { url?: string }) => {
        if (msg.command === "submit") {
          ...
          this._openFile((msg as unknown as { text: string }).text ?? "");
  ...
  private async _onAiDone(repoRoot: string, moduleName: string, reqId: string): Promise<void> {
    ...
    try {
      ...
      const { url, number } = await createGithubIssue(
        token, owner, repo, title, body, ["requirement", "ai-generated"]
      );
