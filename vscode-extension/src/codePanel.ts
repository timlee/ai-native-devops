...
export class CodePanel {
      ...
      (msg: { command: string } & Record<string, unknown>) => {
        switch (msg.command) {
          ...
          case "askAI":
            ...
            break;
          case "openInPlugin":
            this._handleOpenInPlugin((msg.additionalContext as string) ?? "");
            break;
  ...
  private async _handleOpenInPlugin(additionalContext: string): Promise<void> {
    const issue = this._workflowCtx.selectedIssue;
    const branch = this._workflowCtx.branchName;
    if (!issue || !branch) { return; }

    const prompt = buildCodePrompt(issue, branch, additionalContext);
    const provider = vscode.workspace.getConfiguration("aiNativeDevOps").get<string>("provider", "copilot");
