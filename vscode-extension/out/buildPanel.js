...
function detectProject(repoRoot) {
    const probe = (f) => fs.existsSync(path.join(repoRoot, f));
    const detected = [];
    ...
    if (probe("pyproject.toml")) {
        detected.push("pyproject.toml");
    }
    if (probe("go.mod")) {
        detected.push("go.mod");
    }
    if (probe("pom.xml")) {
        detected.push("pom.xml");
    }
    if (probe("build.gradle") || probe("build.gradle.kts")) {
        detected.push("build.gradle");
    }
    if (probe("Cargo.toml")) {
        detected.push("Cargo.toml");
    }
    if (probe("package.json")) {
        return {
...
function deriveValidationCommands(detection) {
...
function buildBuildPrompt(detection, cfg) {
...
function computeRequiredSecrets(cfg) {
    const secrets = [];
    ...
    if (cfg.artifactRegistry === "npm") {
        secrets.push("NPM_TOKEN");
    }
    if (cfg.artifactRegistry === "docker-hub") {
        secrets.push("DOCKER_USERNAME", "DOCKER_PASSWORD");
    }
    if (cfg.artifactRegistry === "github-packages") {
        secrets.push("GITHUB_TOKEN");
    }
    return secrets;
}
function stripYamlFences(raw) {
...
class BuildPanel {
    constructor(_phase, _context, _aiRunner) {
        ...
        this._repoRoot = "";
        this._disposables = [];
        this._panel = vscode.window.createWebviewPanel("buildPanel", _phase.label, vscode.ViewColumn.One, { enableScripts: true, retainContextWhenHidden: true });
            ...
            switch (msg.command) {
                ...
                case "confirmConfig":
                    this._handleConfirmConfig({
                        notificationChannel: msg.notificationChannel ?? "none",
                        slackWebhookSecret: msg.slackWebhookSecret ?? "",
                        emailRecipient: msg.emailRecipient ?? "",
                        artifactRegistry: msg.artifactRegistry ?? "github-packages",
                        customRegistryUrl: msg.customRegistryUrl ?? "",
                        triggerBranches: (msg.triggerBranches ?? "main,develop")
                ...
                case "regeneratePipeline":
                    ...
                    break;
                case "editYaml":
                    this._workflowCtx.editedYaml = msg.yaml ?? "";
                    break;
                case "continueToValidation":
                    this._handleContinueToValidation();
                    break;
                case "runValidation":
                    this._handleRunValidation(msg.index);
                    break;
    ...
    async _handleCommitPipeline() {
        ...
        const exec = (cmd) => new Promise((res, rej) => cp.exec(cmd, { cwd: this._repoRoot, timeout: 30000 }, (err) => (err ? rej(err) : res())));
        // Git operations
        try {
        ...
        try {
            ...
            const owner = repoInfo?.owner ?? settingsCfg.get("githubOwner", "");
            const repo = repoInfo?.repo ?? settingsCfg.get("githubRepo", "");
            if (!owner || !repo) {
                this._panel.webview.postMessage({
                    command: "commitError",
            ...
            const prResult = await (0, githubUtils_1.githubRequest)("POST", `/repos/${owner}/${repo}/pulls`, token, { title: "ci: add automated CI pipeline", body: prBody, head: branchName, base: "main", draft: false });
